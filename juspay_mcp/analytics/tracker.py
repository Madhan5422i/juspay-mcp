...
async def _emit(payload: dict[str, Any], *, mid: str | None = None) -> None:
    ...
    inferred_session_id = await record_event(install_id=ctx.install_id, mid=event_mid, event=event)
    if inferred_session_id is not None:
        logger.info(
            "Analytics stored: mcp=%s type=%s tool=%s status=%s session=%s mid_present=%s",
            ctx.mcp,
            event.get("type"),
            event.get("tool"),
            event.get("status"),
